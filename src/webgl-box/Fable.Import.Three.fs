...
module Three =
   ...
   // custom blending src factors
    ...
    and [<Import("FileLoader","three")>] FileLoader(?manager: LoadingManager) =
        member __.manager with get(): LoadingManager = failwith "JS only" and set(v: LoadingManager): unit = failwith "JS only"
        member __.mimeType with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
        member __.path with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
        member __.requestHeader  with get(): obj = failwith "JS only" and set(v: obj): unit = failwith "JS only"
        member __.responseType with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
        member __.withCredentials with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
        member __.load(url: string, ?onLoad: Func<string, unit>, ?onProgress: Func<obj, unit>, ?onError: Func<obj, unit>): obj = failwith "JS only"
        member __.setMimeType(value: string): FileLoader = failwith "JS only"
        member __.setPath(value: string): FileLoader = failwith "JS only"
        member __.setRequestHeader(value: obj): FileLoader = failwith "JS only"
        member __.setResponseType(value: string): FileLoader = failwith "JS only"
        member __.setWithCredentials(value: bool): FileLoader = failwith "JS only"

    and [<Import("OBJLoader","three")>] OBJLoader(?manager: LoadingManager) =
        member __.load(url: string, ?onLoad: Func<Object3D, unit>, ?onProgress: Func<obj, unit>, ?onError: Func<obj, unit>): obj = failwith "JS only"
        member __.parse(content: string): 'T = failwith "JS only"        

    and [<Import("OBJLoader2","three")>] OBJLoader2(?manager: LoadingManager) =
        member __.load(url: string, ?onLoad: Func<Object3D, unit>, ?onProgress: Func<obj, unit>, ?onError: Func<obj, unit>, ?onMeshAlter: Func<obj, unit>, ?useAsync:bool): obj = failwith "JS only"
        member __.setMaterials(materials: Material): unit = failwith "JS only"
        member __.parse(content: BufferAttribute): 'T = failwith "JS only"
