...
let init() =
    ...
    let color = Three.Color(44.0, 33.0, 49.0)
    ...
    //matProps.color <- Some color

    //let mesh = Three.Mesh(geometry, Three.MeshBasicMaterial(matProps))
    //scene.add mesh

    //let geometry2 = new Three.BoxGeometry(1.0, 1.0, 1.0);
    //let mat = Three.MeshBasicMaterial(matProps);
    //let cube2 = new Three.Mesh( geometry2, mat);
    //scene.add(cube2);

    //let starGeometry = Three.SphereBufferGeometry(500.0, 30.0, 30.0)
    //let starTexture  = Three.ImageUtils.Globals.loadTexture("sunmap.jpg")
    //let starMaterial = Three.MeshPhongMaterial()
    //starMaterial.map <- starTexture
    //let star = Three.Mesh(starGeometry, starMaterial)
    //scene.add(star);

    printfn "%s" "here!!"
    //let onLoad(o:Three.Object3D):unit =
    //  printfn "%s" "here2"
    //  scene.add(o)

    //let loader = Three.OBJLoader2()  // .ObjectLoader()
    //loader.load("box.obj", Func<Three.Object3D,unit>onLoad) |> ignore

    //let onLoadCone(g:Three.Geometry)(m:ResizeArray<Three.Material>):unit =
    //  printfn "%s" "here3"
    //  let material = m.[0]
      //let material = new Three.MeshBasicMaterial()
      //material.color <- Three.Color(44.0, 33.0, 49.0)
      //material.wireframe <- true
    //  let bufferGeometry = new Three.BufferGeometry();
    //  let cone = new Three.Mesh(bufferGeometry.fromGeometry(g), material)
    //  scene.add(cone)

    //let loader2 = Three.JSONLoader()
    //loader2.load("box2.json", Func<Three.Geometry,_,unit> onLoadCone) |> ignore   

    //let onLoad3(o:Three.Object3D):unit =
    //  printfn "%s" "here2"
    //  scene.add(o)

    //let loader3 = Three.ObjectLoader()  // .ObjectLoader()
    //loader3.load("box2.json", Func<Three.Object3D,unit>onLoad3) |> ignore


    let onLoad4(o:Three.Object3D):unit =
      printfn "%s" "here2"
      let mycolor = U2.Case2 "#3C3C3C"
      let ambientLight = new Three.AmbientLight(mycolor, 2.0)
      scene.add(ambientLight)
      //let pointLight = new Three.PointLight(U2.Case2 "#3C3C3C", 1.0, 2.0)
      //scene.add(pointLight)
      o.castShadow <- true
      //o.position.set(0.0, 20.0, 0.0) |> ignore
      
      let directionalLight = new Three.DirectionalLight(U2.Case2 "#3C3C3C", 0.5)
      scene.add(directionalLight)
      let starGeometry = Three.SphereBufferGeometry(500.0, 30.0, 30.0)
      let starTexture  = Three.ImageUtils.Globals.loadTexture("content/crate.gif")
      let starMaterial = Three.MeshPhongMaterial()
      starMaterial.map <- starTexture
      let star = Three.Mesh(starGeometry, starMaterial)
      //scene.add(star);
      //scene.add(cube)
      //let oarray = o.children.[0]
      //let ogeo = oarray.scale
      //let star2 = Three.Mesh(ogeo, starMaterial)
      //scene.add(star2)
      let group = new Three.Group()
      star.position.x <- 300.0
      star.rotation.x <- 2.3 * 360.0 * ( JS.Math.PI / 180.0 )
      star.rotation.y <- 2.4 * 360.0 * ( JS.Math.PI / 180.0 )
      o.scale.set(50.0, 50.0, 50.0) |> ignore
      o.rotation.x <- 2.3 * 360.0 * ( JS.Math.PI / 180.0 )
      o.rotation.y <- 2.4 * 360.0 * ( JS.Math.PI / 180.0 )      
      //group.add(o)
      scene.add(o)

    let loader4 = Three.OBJLoader()
    loader4.load("cone.obj", Func<Three.Object3D,unit>onLoad4) |> ignore

    //let starGeometry2 = Three.SphereBufferGeometry(600.0, 50.0, 50.0)
    //let star2 = Three.Mesh(starGeometry, starMaterial)
    //scene.add(star2);
