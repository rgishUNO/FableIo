...
let init() =
    ...
    // let f = Foo()
    // let rnd = f.Bar(fun (obj: Three.Object3D,u: unit) -> obj :> (Three.Object3D,unit)))
    // let onLoad (ob: Three.Object3D, ret: unit) = (ret)

    //let loader = Three.ObjectLoader()
    //loader.load("bugatti.obj",
    //            Func<Three.Object3D,unit>(fun (o) -> scene.add(o)))

    //let loader = Three.JSONLoader()
    //loader.load("~\\bugatti.js",
    //            Func<Three.Geometry,ResizeArray<Material>,unit>(fun (g) -> scene.add(new Three.Mesh(g,new Three.MeshBasicMaterial(color: 0xff0000)))))
